...
struct PresentCellView: View {
    ...
    @State private var imageUrl: URL? // ✅ Локальный state
    ...
    private func loadImageUrl() {
        Task {
            do {
                let url = try await StorageService.shared.downloadURLPresentImageAsync(id: present.id)
                await MainActor.run {
                    self.imageUrl = url
                }
            } catch {
                print("Не удалось загрузить URL изображения: \(error)")
            }
        }
    }
}
