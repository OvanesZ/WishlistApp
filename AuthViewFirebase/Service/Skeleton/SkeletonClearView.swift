...
struct SkeletonClearView: View {
    @StateObject var viewModel = SkeletonViewModel()

    
    var body: some View {
        let startGradient = Gradient.Stop(color: .clear, location: 0.3)
        let endGradient = Gradient.Stop(color: .clear, location: 0.7)
